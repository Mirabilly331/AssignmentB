...
vwap:{[d;s;T] J:select time,sym,price,vol from T where sym=s; 
...
symvwap:{ [s;T;D] vwap[;s;T] each D; }      

genvwap:{[T] syms:select distinct sym from T;
             dates:select distinct time.date from T;
             symvwap[`GS;T;dates];
             / save `vwapTBL.csv;
        }
...
gendaily each daterange

/ Generate the vwap table 
/ genvwap[stkTBL]
